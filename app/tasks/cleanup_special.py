...
async def cleanup_expired_special():
    """Удаляет просроченных специальных юзеров: сначала из БД, затем из панели."""
    while True:
        try:
            ...
            for user in users:
                ...
                if now > expire_time + timedelta(hours=10):
                    removed_from_db = await hp.remove_special_subscription_by_uuid(uuid)
                    if removed_from_db:
